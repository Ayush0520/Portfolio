...
const ProjectCard = ({ title, description, imageSrc, onSeeMoreClick }) => {
  return (
    <div className="group relative cursor-pointer items-center justify-center overflow-hidden transition-shadow hover:shadow-lg hover:shadow-black/30">
      ...
      <div className="absolute inset-0 flex translate-y-[60%] flex-col items-center justify-center px-9 text-center transition-all duration-500 group-hover:translate-y-0">
        <h3 className="text-3xl font-bold font-serif text-white mb-10">{title}</h3>
        ...
        <button
          ...
          onClick={onSeeMoreClick} // Attach click handler here
        >
          See More
        </button>
      </div>
